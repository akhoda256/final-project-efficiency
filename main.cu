...
float *d_outputCbLPF, *d_outputCrLPF, *d_outputCbLPFInternal, *d_outputCrLPFInternal;
...
void initGPU(int width, int height){
    ...
    cudaMalloc((void**)&d_outputCbLPF, sizeByte);
    cudaMalloc((void**)&d_outputCrLPF, sizeByte);
    cudaMalloc((void**)&d_outputCbLPFInternal, sizeByte);
    cudaMalloc((void**)&d_outputCrLPFInternal, sizeByte);
...
void freeGpu(){
    ...
    cudaFree(d_outputCb);
    cudaFree(d_outputCr);
    cudaFree(d_outputCbLPF);
    cudaFree(d_outputCrLPF);
    gettimeofday (&endtime, NULL);
    double diff = double (endtime.tv_sec) * 1000.0f
                  + double (endtime.tv_usec) / 1000.0f
                  - double (starttime.tv_sec) * 1000.0f
...
__global__ void lowPassKernel1FirstLaunch(float* in, float* out, int width, int height, int offset) {
    float a = 0.25;
    float b = 0.5;
    float c = 0.25;
    ...
    if (x > 0 && x < height - 1 && y > 0 && y < width - 1) {
        // In X
        out[id]= a * in[(x - 1) * width + y] + b * in[x * width + y] + c * in[(x + 1) * width + y];
    }
}

__global__ void lowPassKernel1LastLaunch(float* in, float* out, int width, int height, int offset) {
    float a = 0.25;
    float b = 0.5;
    float c = 0.25;
    int id = blockIdx.x * blockDim.x + threadIdx.x+ offset;
...
__global__ void lowPassKernel1(float* in, float* out, int width, int height, int offset) {
    float a = 0.25;
    float b = 0.5;
    float c = 0.25;
    int id = blockIdx.x * blockDim.x + threadIdx.x+offset;
...
__global__ void lowPassKernel2FirstLaunch(float* in, float* out, int width, int height, int offset) {
    float a = 0.25;
    float b = 0.5;
    float c = 0.25;
    int id = blockIdx.x * blockDim.x + threadIdx.x+offset;
    ...
    if (x > 0 && x < height - 1 && y > 0 && y < width - 1) {
        // In X
        out[id]= a * in[id-1] + b * in[id] + c * in[id+1];
    }
}

__global__ void lowPassKernel2LastLaunch(float* in, float* out, int width, int height, int offset) {
    float a = 0.25;
    float b = 0.5;
    float c = 0.25;
    int id = blockIdx.x * blockDim.x + threadIdx.x+offset;
...
__global__ void lowPassKernel2(float* in, float* out, int width, int height, int offset) {
    float a = 0.25;
    float b = 0.5;
    float c = 0.25;
    int id = blockIdx.x * blockDim.x + threadIdx.x+offset;
...
// Start asynchronous memory copies
    ...
    for (int i = 0; i < numStreams; i++) {
        ...
        int offset2 = offset;
        int height2 = height/numStreams;
        if(i==0){
            lowPassKernel1FirstLaunch<<<gridSize, blockSize, 0, streams[i]>>>(d_outputCb, d_outputCbLPFInternal, width, height2,offset2);
            lowPassKernel2FirstLaunch<<<gridSize, blockSize, 0, streams[i]>>>(d_outputCbLPFInternal, d_outputCbLPF, width, height2, offset2);
        }else if (i == numStreams -1){
            offset2 = offset - (2*width);
            height2 = 2+(height/numStreams);
            dim3 gridSize2((2*width/blockSize.x) + (size / (blockSize.x * numStreams))); // Adjust block sizeByte as needed
            lowPassKernel1LastLaunch<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCb, d_outputCbLPFInternal, width, height2,offset2);
            lowPassKernel2LastLaunch<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCbLPFInternal,d_outputCbLPF, width, height2, offset2);
            offset2 += width;
        }else {
            offset2 = offset- (2*width);
            height2 = 2+(height/numStreams);
            dim3 gridSize2((2*width/blockSize.x) + (size / (blockSize.x * numStreams))); // Adjust block sizeByte as needed
            lowPassKernel1<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCb, d_outputCbLPFInternal, width, height2,offset2);
            lowPassKernel2<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCbLPFInternal,d_outputCbLPF, width, height2, offset2);
            offset2 += width;
        }
        cudaMemcpyAsync(&h_outputCb[offset2], &d_outputCbLPF[offset2], (sizeof(float)*(height+1)*width) / numStreams, cudaMemcpyDeviceToHost, streams[i]);
        ...
        offset2 = offset;
        height2 = height/numStreams;
        if(i==0){
            lowPassKernel1FirstLaunch<<<gridSize, blockSize, 0, streams[i]>>>(d_outputCr, d_outputCrLPFInternal, width, height2,offset2);
            lowPassKernel2FirstLaunch<<<gridSize, blockSize, 0, streams[i]>>>(d_outputCrLPFInternal,d_outputCrLPF, width, height2, offset2);
        }else if (i == numStreams -1){
            offset2 = offset - 2*width;
            height2 = 2+(height/numStreams);
            dim3 gridSize2((width/blockSize.x) + (size / (blockSize.x * numStreams))); // Adjust block sizeByte as needed
            lowPassKernel1LastLaunch<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCr, d_outputCrLPFInternal, width, height2,offset2);
            lowPassKernel2LastLaunch<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCrLPFInternal,d_outputCrLPF, width, height2, offset2);
            offset2 += width;
        }else {
            offset2 = offset- 2*width;
            height2 = 2+(height/numStreams);
            dim3 gridSize2((width/blockSize.x) + (size / (blockSize.x * numStreams))); // Adjust block sizeByte as needed
            lowPassKernel1<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCr, d_outputCrLPFInternal, width, height2,offset2);
            lowPassKernel2<<<gridSize2, blockSize, 0, streams[i]>>>(d_outputCrLPFInternal,d_outputCrLPF, width, height2, offset2);
            offset2 += width;
        }
        cudaMemcpyAsync(&h_outputCr[offset2], &d_outputCrLPF[offset2], (sizeof(float)*(height+1)*width)/numStreams, cudaMemcpyDeviceToHost, streams[i]);
        //cudaMemcpyAsync(&h_outputY[offset], &d_outputY[offset], sizeByte / numStreams, cudaMemcpyDeviceToHost, streams[i]);
...
downSample (Channel *in)
{
  ...
  Channel *out = new Channel ((width / 2), (height / 2));
    ...
    #pragma omp parallel for num_threads(4)
    for (int x2 = 0; x2 < h2; x2++)
    {
        for (int y2 = 0; y2 < w2; y2++)
        {
          out->data[x2 * w2 + y2] = in->data[(x2*2) * width + 2*y2];
        }
    }
...
//    }
    ...
    for (int i = 0; i < width*height; i += 16) {
        // Load a vector of 4 floats (AVX) from the input data
        __m512 inData = _mm512_loadu_ps(&in->data[i]); // For AVX, use _mm256_loadu_ps
        // Subtract 128 from each element
        inData = _mm512_sub_ps(inData, _mm512_set1_ps(128.0f)); // For AVX, use _mm256_sub_ps

        // Create a vector of zeros
        __m512 zeroVector = _mm512_setzero_ps(); // For AVX, use _mm256_setzero_ps

        // Store the result back in the output data
        _mm512_storeu_ps(&in->data[i], inData); // For AVX, use _mm256_storeu_ps
        _mm512_storeu_ps(&out->data[i], zeroVector); // Set elements to zero
    }
  ...
  for (int i = 0; i < height; i++)
    {
      //out->data[i] = 0; // zeros
      int offset = i * width;
      memset(&out->data[offset], 0, width*sizeof(float));
    }
...
cpyBlock (float *in, float *out, int blocksize, int stride)
{
    ...
    {
        __m256 data = _mm256_loadu_ps(&in[i*stride]);
        _mm256_storeu_ps(&out[i*blocksize], data);
    ...
    for (int x = 0; x < height; x += 8)
    {
        for (int y = 0; y < width; y += 8)
        {
          ...
          for (int index = 0; index < MPEG_CONSTANT; index++)
            ...
            memcpy(&ordered->data[blockNumber*MPEG_CONSTANT], zigZagOrdered, MPEG_CONSTANT*sizeof(float));
  ...
  for (int i = 0; i < num_blocks; i++)
    {
      ...
      for (int y = 0; y < width; y+=8){
          ...
          __m256 data = _mm256_load_ps(&ordered->data[i * width + y]);
          __m512d double_variable = _mm512_cvtps_pd(data);
          _mm512_storeu_pd(&block[y], double_variable);
      }


      //std::copy(&ordered->data[i*width], &ordered->data[i*width]+width , block);
              ...
              if (in_zero_run == 1)
                {
                  ...
                  block_encode[encoded_index]
                      = "Z" + boost::lexical_cast<string>(zero_count);//std::to_string (zero_count);
                  encoded_index = encoded_index + 1;
                }
              block_encode[encoded_index] = boost::lexical_cast<string>((int)coeff);
      ...
      if (in_zero_run == 1)
        {
          if (zero_count > 1)
            {
              block_encode[encoded_index] = "Z" +  boost::lexical_cast<string>(zero_count);;
            }
          else
            {
  ...
  for (int frame_number = 0; frame_number < end_frame; frame_number++)
    {
      ...
      frame_lowpassed->Cb->copy (frame_ycbcr->gc);
      frame_lowpassed->Cr->copy (frame_ycbcr->bc);
      gettimeofday (&endtime, NULL);
