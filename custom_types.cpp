...
float round(float number){
    return number < 0.0 ? (float)ceil(number - 0.5) : (float)floor(number +
0.5);
}
*/
...
Channel::copy (Channel *ch)
{
  ...
  for(int i = 0; i < chunkSize; i++){
      int offset = i*(npixels / chunkSize);
      memcpy(&this->data[offset], &ch->data[offset], (npixels*sizeof(float))/chunkSize);
  }
